...
(Python — geração das configurações via Latin Hypercube Sampling, execução
...
| `useServerInfoQuery` | `GET /api/server-info` | uma vez (`staleTime: Infinity`) | Informação estática da máquina (modelo de CPU, sensores disponíveis) — não muda durante a sessão, então não há por que repetir a requisição. |
